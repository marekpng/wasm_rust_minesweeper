...
//mod random;
use std::{collections::HashSet, sync::PoisonError, fmt::{Display, Write}};
        ...
        for y in 0..self.height {
            for x in 0..self.width {
                ...
                if !self.open_fields.contains(&pos) {
                    ...
                    let mine_count = self.neighboring_mine(pos);
                    if mine_count > 0 {
                        write!(f, " {} ", mine_count)?;
...
impl Minesweeper {
    ...
    pub fn neighboring_mine(&self, pos: Position) -> u8 {
    ...
    pub fn open(&mut self, position: Position) -> Option<OpenResult> {
        ...
        if is_mine {
            ...
            let mine_count = self.neighboring_mine(position);
            if mine_count == 0{
